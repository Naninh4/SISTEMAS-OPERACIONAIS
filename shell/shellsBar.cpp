...
std::string getHostName() {
    ...
    if (gethostname(hostname, sizeof(hostname)) != 0) {
        std::strncpy(hostname, "unknown-host", sizeof(hostname) - 1);
    }

    return std::string(hostname);  // Retorna como std::string
}
std::string host = getHostName();


std::string def = PINK + (user ? std::string(user) : "unknown-user") + RESET + "@" + PURPLE + host + RESET + ":";
std::string PS1 = def;
